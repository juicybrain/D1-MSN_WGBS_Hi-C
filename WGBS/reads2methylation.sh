...
# call methylation and calculate the conversion rate
bismark_methylation_extractor -p --bedGraph --gzip --CX --ignore_3prime 3 --ignore_3prime_r2 3 --cytosine_report --genome_folder ~/ref/genome/lambdaDNA/  ${spl}\_filter.bam
      ...
      cat CA.lambda.cov  |awk 'BEGIN{me=0;unme=0;OFS="\t"}{me=me+$4;unme=unme+$5}END{print "CA", me, unme,1-me/(me+unme)}' >> conversion_rate.CX.txt
      cat CC.lambda.cov  |awk 'BEGIN{me=0;unme=0;OFS="\t"}{me=me+$4;unme=unme+$5}END{print "CC", me, unme,1-me/(me+unme)}' >> conversion_rate.CX.txt
      cat CT.lambda.cov  |awk 'BEGIN{me=0;unme=0;OFS="\t"}{me=me+$4;unme=unme+$5}END{print "CT", me, unme,1-me/(me+unme)}' >> conversion_rate.CX.txt
      cat CG.lambda.cov  |awk 'BEGIN{me=0;unme=0;OFS="\t"}{me=me+$4;unme=unme+$5}END{print "CG", me, unme,1-me/(me+unme)}' >> conversion_rate.CX.txt
